// Auto-generated feature route: conservation-autonomous
// Domain: autonomous conservation predictor — Capability: predicting deterioration
// Mounted under: /api/conservation-autonomous
...
// POST /api/conservation-autonomous/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a autonomous conservation predictor. Your task is predicting deterioration. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/conservation-autonomous/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/conservation-autonomous/:id — fetch one
...
// DELETE /api/conservation-autonomous/:id — discard a run

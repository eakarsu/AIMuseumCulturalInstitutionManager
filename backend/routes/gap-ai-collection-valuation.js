...
// Route: /api/gap-ai-collection-valuation
// Feature: Ai Collection Valuation
// Description: (artifact market values)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Collection Valuation. (artifact market values)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Collection Valuation\nDescription: (artifact market values)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-collection-valuation', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-collection-valuation', title: 'Ai Collection Valuation', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-collection-valuation', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-collection-valuation']);
      return res.json({ slug: 'ai-collection-valuation', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-collection-valuation', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-collection-valuation', items: [], error: (e && e.message) || 'Server error' });
  }
});

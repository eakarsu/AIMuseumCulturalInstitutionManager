// Auto-generated feature route: collection-curator-agent
// Domain: agentic museum curator — Capability: suggesting exhibition concepts and educational narratives
// Mounted under: /api/collection-curator-agent
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Museum Cultural Institution Manager'
      }
    };
...
// POST /api/collection-curator-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic museum curator. Your task is suggesting exhibition concepts and educational narratives. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/collection-curator-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/collection-curator-agent/:id — fetch one
...
// DELETE /api/collection-curator-agent/:id — discard a run

// Auto-generated feature route: visitor-journey-agent
// Domain: personalized visitor journey agent — Capability: real-time wayfinding and recommendations
// Mounted under: /api/visitor-journey-agent
...
// POST /api/visitor-journey-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a personalized visitor journey agent. Your task is real-time wayfinding and recommendations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/visitor-journey-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/visitor-journey-agent/:id — fetch one
...
// DELETE /api/visitor-journey-agent/:id — discard a run

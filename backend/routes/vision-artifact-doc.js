// Auto-generated feature route: vision-artifact-doc
// Domain: artifact vision documentation agent — Capability: extracting photo + measurement + condition data
// Mounted under: /api/vision-artifact-doc
...
// POST /api/vision-artifact-doc/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a artifact vision documentation agent. Your task is extracting photo + measurement + condition data. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-artifact-doc/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-artifact-doc/:id — fetch one
...
// DELETE /api/vision-artifact-doc/:id — discard a run

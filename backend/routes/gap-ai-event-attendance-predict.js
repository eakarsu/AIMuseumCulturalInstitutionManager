...
// Route: /api/gap-ai-event-attendance-predict
// Feature: Ai Event Attendance Predict
// Description: (forecast event attendance)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Event Attendance Predict. (forecast event attendance)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Event Attendance Predict\nDescription: (forecast event attendance)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-event-attendance-predict', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-event-attendance-predict', title: 'Ai Event Attendance Predict', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-event-attendance-predict', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-event-attendance-predict']);
      return res.json({ slug: 'ai-event-attendance-predict', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-event-attendance-predict', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-event-attendance-predict', items: [], error: (e && e.message) || 'Server error' });
  }
});
